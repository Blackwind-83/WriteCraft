...
export class FilePropertyManager {
    ...
    static async readProperty(file: TFile, propertyName: string): Promise<string | string [] | undefined> {
        try {
            ...
            const content = await FilePropertyManager.plugin.app.vault.cachedRead(file);

            // Appeler readFrontMatter pour extraire le frontmatter
            const frontmatter = await FilePropertyManager.readFrontMatter(content);
    ...
    static async updateProperty(file: TFile, key: string, value: string | string [] | boolean) {
        const content = await FilePropertyManager.plugin.app.vault.read(file);
        const updatedContent = FilePropertyManager.modifyFrontmatter(content, key, value);
        await FilePropertyManager.plugin.app.vault.modify(file, updatedContent);
